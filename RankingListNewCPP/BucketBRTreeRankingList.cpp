...
// UserBucket 类实现
BucketBRTreeRankingList::UserBucket::UserBucket()
    : Users(BUCKET_SIZE), UserCount(0)
{}

BucketBRTreeRankingList::UserBucket::UserBucket(const std::vector<User>& users, int userCount)
    : Users(users), UserCount(userCount)
{}

int BucketBRTreeRankingList::UserBucket::IndexOf(const User& user) const
{
    auto it = std::lower_bound(Users.begin(), Users.begin() + UserCount, user);
    if (it != Users.begin() + UserCount && *it == user)
    {
        return it - Users.begin();
    }
    return -1;
}
...
{
    int index = std::lower_bound(Users.begin(), Users.begin() + UserCount, user) - Users.begin();
    
    // 移动元素为新用户腾出空间
    std::move_backward(Users.begin() + index, Users.begin() + UserCount, Users.begin() + UserCount + 1);
    Users[index] = user;
    UserCount++;
...
int BucketBRTreeRankingList::UserBucket::Remove(const User& user)
{
    ...
    // 移动元素覆盖要删除的用户
    std::move(Users.begin() + index + 1, Users.begin() + UserCount, Users.begin() + index);
    UserCount--;
...
{
    int mid = UserCount / 2;
    userIndex = std::lower_bound(Users.begin(), Users.begin() + UserCount, user) - Users.begin();
    ...
    if (userIndex >= mid)
    {
        ...
        std::copy(Users.begin() + mid, Users.begin() + userIndex, newBucket->Users.begin());
        // 将新用户插入到新桶的userIndex - mid位置
        newBucket->Users[userIndex - mid] = user;
        // 将userIndex到UserCount-1的元素复制到新桶
        std::copy(Users.begin() + userIndex, Users.begin() + UserCount, newBucket->Users.begin() + userIndex - mid + 1);
        newUserCount++;
    }
    else
    {
        // 将mid到UserCount-1的元素复制到新桶
        std::copy(Users.begin() + mid, Users.begin() + UserCount, newBucket->Users.begin());
    }
...
{
    // 将other的元素复制到当前桶的末尾
    std::copy(other->Users.begin(), other->Users.begin() + other->UserCount, Users.begin() + UserCount);
    UserCount += other->UserCount;
}

// BucketBRTreeRankingList 类实现
std::vector<BucketBRTreeRankingList::UserBucket*> BucketBRTreeRankingList::BuildBucket(std::vector<User>& users)
{
    std::sort(users.begin(), users.end());
    
    int bucketNum = static_cast<int>(std::ceil(static_cast<double>(users.size()) / INITIAL_BUCKET_SIZE));
    std::vector<UserBucket*> buckets(bucketNum);
    
    for (int i = 0; i < bucketNum; i++)
    {
        int l = i * INITIAL_BUCKET_SIZE;
        int r = std::min((i + 1) * INITIAL_BUCKET_SIZE, static_cast<int>(users.size()));
        int userCount = r - l;
        
        std::vector<User> bucketUsers(BUCKET_SIZE);
        std::copy(users.begin() + l, users.begin() + r, bucketUsers.begin());
...
BucketBRTreeRankingList::BucketBRTreeRankingList(const std::vector<User>& users)
    : _root(new TreeNode())
{
    std::vector<User> sortedUsers = users;
    std::sort(sortedUsers.begin(), sortedUsers.end());
    
    std::vector<UserBucket*> buckets = BuildBucket(sortedUsers);
...
User BucketBRTreeRankingList::GetUserRank(int userId) const
{
    ...
    // 返回用户，但注意：这里没有修改User结构体来存储排名
    return user;
}

int BucketBRTreeRankingList::GetTopN(int topN, User* pOutUsers) const
{
    ...
    // 复制前n个用户到输出数组
    std::copy(bucket->Users.begin(), bucket->Users.begin() + n, pOutUsers + rankCount);
    rankCount += n;
...
int BucketBRTreeRankingList::GetArroundUser(int userId, int arroundN, User* pOutUsers) const
{
    ...
    std::copy(bucket->Users.begin() + userIndexInBucket - leftCount, 
              bucket->Users.begin() + userIndexInBucket + rightCount + 1, 
              result.begin() + arroundN - leftCount + offset);
    ...
    while (leftCount < leftNum)
    {
        ...
        std::copy(bucket->Users.begin() + bucket->UserCount - n, 
                  bucket->Users.begin() + bucket->UserCount, 
                  result.begin() + arroundN - leftCount - n + offset);
    ...
    while (rightCount < rightNum)
    {
        ...
        std::copy(bucket->Users.begin(), bucket->Users.begin() + n, 
                  result.begin() + arroundN + rightCount + 1 + offset);
...
int BucketBRTreeRankingList::GetUserCount() const
{
    return _root->Count;
}
