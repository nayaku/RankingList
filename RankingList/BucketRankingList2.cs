        ...
        internal class UserBucket
        {
            ...
            public void Combine(UserBucket[] other)
            {
                foreach (UserBucket bucket in other)
                {
                    ...
                    foreach (IUser user in bucket.UserIds.Values)
                    {
                        UserIds.Add(user.Id, user);
                    }
                }
...
内存占用: 648.56 MB
内存峰值: 648.56 MB
测试日期: 2026/1/20 16:02:38

=== 与基准 BucketRankingList 的对比 ===
总耗时: 3134 ms vs 3246 ms (-3.45%)
平均耗时: 0.00 ms vs 0.00 ms (-3.45%)
内存占用: 648.56 MB vs 645.94 MB (+0.41%)
内存峰值: 648.56 MB vs 645.94 MB (+0.41%)
*/