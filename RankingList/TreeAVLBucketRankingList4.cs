...
{
    public class TreeAVLBucketRankingList4 : IRankingList
    {
        ...
        public TreeAVLBucketRankingList4(IUser[] users)
        {
            Array.Sort(users);
        ...
        private static TreeNode OperateTree(TreeNode node, IUser user, ref int rankCount, bool isAdd, ref bool needRebalance)
        {
            // 叶子节点
            ...
            {
                node.Left = OperateTree(node.Left, user, ref rankCount, isAdd, ref needRebalance);
                node.LeftUser = node.Left.LeftUser;
            }
            else
            {
                rankCount += node.Left.Count;
                node.Right = OperateTree(node.Right, user, ref rankCount, isAdd, ref needRebalance);
                node.RightUser = node.Right.RightUser;
            }
            ...
            Debug.Assert(node.Height == Math.Max(node.Left.Height, node.Right.Height) + 1, "高度计算错误");
            return node;
        }
        ...
        public RankingListResponse AddUser(IUser user)
        {
            ...
            _root = OperateTree(_root, user, ref rankCount, true, ref needRebalance);
        ...
        public RankingListResponse UpdateUser(IUser user)
        {
            ...
            bool needRebalance = false;
            _root = OperateTree(_root, oldUser, ref rankCount, false, ref needRebalance);
            rankCount = 0;
            _root = OperateTree(_root, user, ref rankCount, true, ref needRebalance);
...
=== 测试 TreeAVLBucketRankingList4 排行榜 ===
初始用户数: 10000
操作数: 1000000

=== 验证操作结果与基准对比 ===
√ 所有操作结果验证通过！
测试操作结果已保存到 TreeAVLBucketRankingList4_test_results.json

=== 测试结果 ===
排行榜名称: TreeAVLBucketRankingList4
总耗时: 1826 ms
平均耗时: 1.83 ms/1000操作
内存占用: 710.55 MB
内存峰值: 709.57 MB
测试日期: 2026/1/28 10:04:50

=== 与基准 BucketRankingList 的对比 ===
总耗时: 1826 ms vs 3122 ms (-41.51%)
平均耗时: 1.83 ms/1000操作 vs 3.12 ms/1000操作 (-41.51%)
内存占用: 710.55 MB vs 707.04 MB (+0.50%)
...
【AddUser】
  ...
  总耗时: 57 ms vs 190 ms (-70.00%) (100000次操作)
  平均耗时: 0.57 ms/1000操作 vs 1.90 ms/1000操作 (-70.00%)
【UpdateUser】
  操作数: 200000 vs 200000
  总耗时: 353 ms vs 2874 ms (-87.72%) (200000次操作)
  平均耗时: 1.77 ms/1000操作 vs 14.37 ms/1000操作 (-87.72%)
【GetUserRank】
  操作数: 300000 vs 300000
  总耗时: 250 ms vs 2313 ms (-89.19%) (300000次操作)
  平均耗时: 0.83 ms/1000操作 vs 7.71 ms/1000操作 (-89.19%)
【GetTopN】
  操作数: 200000 vs 200000
  总耗时: 504 ms vs 608 ms (-17.11%) (200000次操作)
  平均耗时: 2.52 ms/1000操作 vs 3.04 ms/1000操作 (-17.11%)
【GetAroundUser】
  操作数: 200000 vs 200000
  总耗时: 290 ms vs 1765 ms (-83.57%) (200000次操作)
  平均耗时: 1.45 ms/1000操作 vs 8.83 ms/1000操作 (-83.57%)
*/
// 没啥区别