...
{
    public class TreeAVLBucketRankingList3 : IRankingList
    {
        ...
        private static readonly ObjectPool<TreeNode> _nodePool = new();

        public TreeAVLBucketRankingList3(IUser[] users)
        {
            Array.Sort(users);
        ...
        private static TreeNode OperateTree(TreeNode node, IUser user, ref int rankCount, bool isAdd)
        {
            ...
            {
                // 左子树为空，用右子树代替
                _nodePool.Return(node.Left);
                TreeNode right = node.Right;
                node.CopyFrom(right);
                _nodePool.Return(right);
            }
            else if (node.Right.Empty)
            {
                // 右子树为空，用左子树代替
                _nodePool.Return(node.Right);
                TreeNode left = node.Left;
                node.CopyFrom(left);
                _nodePool.Return(left);
            }
...
=== 测试 TreeAVLBucketRankingList3 排行榜 ===
初始用户数: 10000
操作数: 1000000

=== 验证操作结果与基准对比 ===
√ 所有操作结果验证通过！
测试操作结果已保存到 TreeAVLBucketRankingList3_test_results.json

=== 测试结果 ===
排行榜名称: TreeAVLBucketRankingList3
总耗时: 1817 ms
平均耗时: 1.82 ms/1000操作
内存占用: 709.92 MB
内存峰值: 709.92 MB
测试日期: 2026/1/28 9:16:55

=== 与基准 BucketRankingList 的对比 ===
总耗时: 1817 ms vs 3122 ms (-41.80%)
平均耗时: 1.82 ms/1000操作 vs 3.12 ms/1000操作 (-41.80%)
内存占用: 709.92 MB vs 707.04 MB (+0.41%)
内存峰值: 709.92 MB vs 707.04 MB (+0.41%)

=== 单项操作耗时测试 ===

【AddUser】
  操作数: 100000 vs 100000
  总耗时: 52 ms vs 190 ms (-72.63%) (100000次操作)
  平均耗时: 0.52 ms/1000操作 vs 1.90 ms/1000操作 (-72.63%)
【UpdateUser】
  操作数: 200000 vs 200000
  总耗时: 352 ms vs 2874 ms (-87.75%) (200000次操作)
  平均耗时: 1.76 ms/1000操作 vs 14.37 ms/1000操作 (-87.75%)
【GetUserRank】
  操作数: 300000 vs 300000
  总耗时: 246 ms vs 2313 ms (-89.36%) (300000次操作)
  平均耗时: 0.82 ms/1000操作 vs 7.71 ms/1000操作 (-89.36%)
【GetTopN】
  操作数: 200000 vs 200000
  总耗时: 501 ms vs 608 ms (-17.60%) (200000次操作)
  平均耗时: 2.51 ms/1000操作 vs 3.04 ms/1000操作 (-17.60%)
【GetAroundUser】
  操作数: 200000 vs 200000
  总耗时: 297 ms vs 1765 ms (-83.17%) (200000次操作)
  平均耗时: 1.49 ms/1000操作 vs 8.83 ms/1000操作 (-83.17%)
*/
// 加入对象池，没啥区别